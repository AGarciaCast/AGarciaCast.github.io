---
title: "Beyond RGB: Exploring Color Equivariance in Neural Networks"
date: "2024-06-30"
summary: "What if CNNs understood color transformations as naturally as spatial ones? 🌈 We explore Color Equivariant Networks, extend them beyond hue shifts into HSV and LAB color spaces, and uncover how color symmetries can improve robustness and image classification performance."
tags:
  - Demo
  - External
draft: false
...
<meta http-equiv="refresh" content="0;url=https://github.com/AGarciaCast/CEConv/blob/main/blogpost.md">
<script>window.location.replace("https://github.com/AGarciaCast/CEConv/blob/main/blogpost.md");</script>

Si no se redirige automáticamente, haz clic en este enlace: <a href="https://github.com/AGarciaCast/CEConv/blob/main/blogpost.md/">Ir al post externo</a>.
...
In Figure 2, the accuracy improvement of color equivariance up to later stages in the network is displayed for the aforementioned datasets. The baseline is the ResNet-18 model trained for one hue rotation, which encodes a 0-degree hue shift. For the other values, HybridResNet-18 models are trained with 3 rotations, max pooling, separable kernels, and the number of color equivariant stages as shown in the figure. Additionally, the graph on the right shows the result with color-jitter augmentation.
...
The main implementation of color equivariance consists of adding three rotations of 120 degrees, whereas the baseline model (without equivariance) can be expressed as having one rotation. In Figure 4, we reproduced the experiments examining the effect of additional rotations on the CE-ResNet-18. In order to examine the full potential of the model, we did not reduce the width of the network and instead provide an insight into the increase in the number of parameters (see Table 1). To save computational power, we limited the experiments to 1, 5, and 10 rotations (instead of 1-10 in the original paper). The models are evaluated across 37 discrete test-time hue shifts, similar to the previous experiment.
...
$$
(10) \quad H_n(k) = \\begin{bmatrix} \frac{2k\pi}{n} \\\\ 0 \\\\ 0 \\end{bmatrix} 
$$
...
$$
(11) \quad \[H_n(k)f\] (x) = \\begin{bmatrix} (f(x)_h + \frac{2k\pi}{n}) \\% 2\pi \\\\ f(x)_s \\\\ f(x)_v \\end{bmatrix}
$$
...
$$
(13) \quad \[\mathcal{L}_{t, m}f\](x) = \[H_n(m)f\](x-t) = \\begin{bmatrix} (f(x - t)_h + \frac{2\pi}{n} m) \\% 2\pi \\\\ f(x - t)_s \\\\ f(x - t)_v \\end{bmatrix}
$$

We can then define the lifting layer outputting the $i$-th output channel as:

$$
(14) \quad \[\psi^i \star f \](x, k) = \sum_{y \in \mathbb{Z}^2} \psi^i(y) \cdot \[H_n(k)f\](y-x) 
$$
...
$$
(16) \quad \[f \star\psi^i\](x, k) = \sum_{y \in \mathbb{Z}^2} \sum_{r=1}^n f(y,r) \cdot \psi^i(y-x, (r-k)\\% n)
$$
...
$$
(19) \quad \[H_n(k)f\](x) = \\begin{bmatrix} f(x)_h \\\\ f(x)_s \\\\ \text{clip}(0, f(x)_v + (-1 + k \frac{2}{n-1}), 1) \\end{bmatrix}
$$
...
$$ 
(20) \quad G = \mathbb{Z}_2 \times C_n \times \mathbb{R} \times \mathbb{R} 
$$
...
$$
(21) \quad \mathcal{L}_{(t, h',s',v')} = \[H_n(h',s',v')f\](x-t) = 
